...
<main class="min-h-screen grid place-items-center px-4">
	<div class="w-full max-w-md border border-cyan-500/30 bg-black/30 rounded-2xl p-6">
		...
		<p class="text-sm text-gray-400 mb-5">[{mode}] connect-to-service</p>
		...
		<button class="btn-primary w-full mt-4" on:click={submit} disabled={loading}>
			{loading ? '...' : mode}
		</button>

		<button class="text-sm text-cyan-400 hover:underline mt-3" on:click={() => (mode = mode === 'login' ? 'register' : 'login')}>
