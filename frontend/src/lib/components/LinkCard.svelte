...
>
    <div class="flex items-start justify-between gap-2 min-h-0 overflow-hidden">
        <div class="min-w-0 flex-1 overflow-hidden">
            ...
            {#if link.description}
                <p class="text-gray-400 text-sm mt-2 line-clamp-2 break-words">{link.description}</p>
            {/if}
        </div>
        ...
        {#if onEdit || onDelete}
            <div class="flex items-center gap-1.5 shrink-0 self-start">
                {#if onEdit}
                    <button
                        type="button"
                        class="px-2 py-1 text-xs leading-none rounded border border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/15 transition whitespace-nowrap"
                ...
                {#if onDelete}
                    <button
                        type="button"
                        class="px-2 py-1 text-xs leading-none rounded border border-red-500/35 text-red-300 hover:bg-red-500/15 transition whitespace-nowrap"
